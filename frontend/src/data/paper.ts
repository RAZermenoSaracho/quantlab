...
function mergeTrade(existing: PaperTrade, payload: TradeExecution): PaperTrade {
  return {
    ...
    entry_notional:
      payload.entry_notional ?? existing.entry_notional ?? null,
    exit_notional:
      payload.exit_notional ?? existing.exit_notional ?? null,
    entry_fee: payload.entry_fee ?? existing.entry_fee ?? null,
    exit_fee: payload.exit_fee ?? existing.exit_fee ?? null,
    total_fee: payload.total_fee ?? existing.total_fee ?? null,
    gross_pnl: payload.gross_pnl ?? existing.gross_pnl ?? null,
    net_pnl: payload.net_pnl ?? existing.net_pnl ?? null,
    fee_rate_used:
      payload.fee_rate_used ?? existing.fee_rate_used ?? null,
...
function makeTrade(payload: TradeExecution): PaperTrade {
  ...
  return {
    ...
    quantity: payload.quantity,
    entry_notional: payload.entry_notional ?? null,
    exit_notional: payload.exit_notional ?? null,
    entry_fee: payload.entry_fee ?? null,
    exit_fee: payload.exit_fee ?? null,
    total_fee: payload.total_fee ?? null,
    gross_pnl: payload.gross_pnl ?? null,
    net_pnl: payload.net_pnl ?? null,
    fee_rate_used: payload.fee_rate_used ?? null,
